...
func (oq *OrderQuery) QueryOwner() *UserQuery {
...
func (oq *OrderQuery) WithOwner(opts ...func(*UserQuery)) *OrderQuery {
	query := &UserQuery{config: oq.config}
...
func (oq *OrderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Order, error) {
	...
	if oq.withOwner != nil || oq.withProduct != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := oq.withOwner; query != nil {
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_orders" returned %v`, n.ID)
			}
			for i := range nodes {
